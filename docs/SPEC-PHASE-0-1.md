...
> **Версия:** 1.2 — administration как %-надбавка, статусы оплат, контакты клиента (D30–D34)
> **Основание:** DISCOVERY.md v0.6 (решения D1–D34, раунды 1–4 закрыты)
> **Дата:** 2026-07-09
...
create table app.clients (
  ...
  name text not null,
  contact jsonb,
  admin_percent numeric(5,2) not null default 18,  -- надбавка administration (D30)
  created_at timestamptz not null default now()
);

create table app.client_contacts (      -- art manager'ы клиента (D32)
  id uuid primary key default gen_random_uuid(),
  client_id uuid not null references app.clients(id),
  full_name text not null,
  role text,                             -- 'art manager'
...
create table app.board_mappings (
  ...
  billing_class text not null
    ...
    -- administration в Kaiten НЕ существует: это %-надбавка клиента (D30)
...
Модель следует реальному артефакту (образец «MimirHead to Gaijin», D25–D34):
инвойс — на **клиента**, охватывает несколько проектов; строка клиенту — **ассет**
(родительская карточка-«оболочка», часы в child-карточках-этапах) с pivot'ом часов
по типам работ; administration — **не данные, а формула**: `admin_percent` клиента
(сейчас 18%) × заапрувленные часы ассетов, по каждому проекту; клиентский апрув —
на уровне строки-ассета.
...
create table app.billing_periods (       -- клиент × период (D25)
  ...
  admin_percent numeric(5,2) not null,   -- снапшот clients.admin_percent (D30)
...
create table app.billing_items (
  ...
  asset_card_id bigint not null,         -- родительская карточка-«оболочка»
  asset_title text not null,             -- снапшот названия ассета
  task_type text references app.task_types(code),
  kaiten_card_id bigint not null,        -- child-карточка-этап (источник логов)
  employee_id uuid references app.employees(id),
...
create unique index on app.billing_items
  (billing_period_id, asset_card_id, kaiten_card_id,
   coalesce(task_type,''), coalesce(kaiten_user_id,0));

-- Статус строки-ассета глазами клиента (чекбокс approved, art manager; D28, D32)
create table app.billing_asset_status (
  ...
  specification text,                    -- автогенерация из task_types + ручная правка
...
create table app.invoices (              -- на клиента, мультипроектный (D25)
  ...
  currency text not null,
  hours_total numeric(9,2) not null,     -- включая administration
  amount_total numeric(12,2) not null,   -- Σ по проектам: часы × ставка (снапшот)
  status text not null default 'draft' check (status in
    ('draft','sent','signed','in_payment','paid')),   -- D31
...
`hours_internal := hours_raw`. Пересборка пула возможна только в
`draft`/`internal_review` и не трогает введённые корректировки (merge по ключу).

**Administration** (D30): не хранится построчно — вычисляется:
`admin_hours(project) = round(billing_periods.admin_percent% × Σ hours_client
заапрувленных ассетов проекта)`. Попадает в экспорт акта (строка
`administration <Project>`) и в инвойс (входит в часы строки проекта).

`specification` для строки-ассета генерируется из фактических типов работ
(+ ручное редактирование перед экспортом), хранится в `billing_asset_status`.
...
| `/billing/[id]` | owner, pm, lead* | Двухтабличный экран по прототипу владельца (D36), см. ниже |
...
**Экран `/billing/[id]`** (контракт из прототипа `tasktrackerfull.jsx`, D36):
- **Таблица «Done»**: дерево ассет → стадии (child-карточки) с часами; чекбоксы
  с каскадом вниз и tri-state вверх (все стадии выбраны ⇒ выбран ассет);
  сортировка по проекту/исполнителю/часам; кнопка «Отправить в Review».
- **Таблица «Review»**: строка = ассет; колонки: итого + pivot по стадиям
  (**все стадии обоих пайплайнов**, D35); ассет группируется, только если все его
  стадии в review, иначе стадии — отдельными строками; редактируемое описание
  (specification); менеджер (popover-выбор); действия «← Вернуть» (в т.ч. «Вернуть
  все») и «✓ Approve»; футер — итоги по каждой колонке; кнопка экспорта.
- Все переносы Done↔Review и Approve пишутся в audit_log.

**Экспорт 1 — акт часов** (XLSX, воспроизводит образец «MimirHead to Gaijin»):
...
(= admin_percent × апрувнутые часы), `total <Project>`, общий `total`, `total paid`.
Денег в акте нет; чекбоксы оплат рендерятся из статуса инвойса (D31).

**Экспорт 2 — финальный инвойс** (XLSX): строки-задачи:
`название | проект | изображение ассета | часы | сумма (часы × ставка)`;
последняя строка — `Администрирование`; итоги часов по проектам; общий итог USD.
Изображение: v1 — пустая ячейка (вставляется вручную), v2 — обложка карточки
из Kaiten API (D33).

PM вставляет экспорт в шаблон клиента; интеграция Sheets API — Фаза 5 (D18).
...
2. **Создать два проекта Supabase** (prod, staging) — инструкция: `docs/SUPABASE_SETUP.md`;
   прислать Project Ref'ы. ← единственный блокер
3. ~~Образец инвойса~~ — ✅ получен; ~~вопросы раунда 4~~ — ✅ отвечены (D30–D34).
4. Подтвердить D18 (экспорт XLSX как v1 инвойса).
5. Остаточные вопросы Фаз 3–4 (больничные %, сдвиг отпуска, налоговые ставки) —
